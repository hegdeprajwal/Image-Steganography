...
class IMG_Stegno:
    ...
    def main(self, root):
        ...
        title = Label(frame,text='Image Steganography Project')
        title.config(font=('Times new roman',25, 'bold'))
        title.grid(pady=10)
        title.config(bg = '#010101')
        title.grid(row=1)

        encode = Button(frame,text="Encode Image",command= lambda :self.encode_frame1(frame), padx=14,bg = '#e3f4f1' )
        encode.config(font=('Helvetica',14), bg='#e8c1c7')
        encode.grid(row=2)
        decode = Button(frame, text="Decode Image",command=lambda :self.decode_frame1(frame), padx=14,bg = '#e3f4f1')
    ...
    def encode_frame1(self,F):
        ...
        label1.config(font=('Times new roman',25, 'bold'),bg = '#1b7ced')
        label1.grid()
    ...
    def encode_frame2(self,e_F2):
        ...
        if not myfile:
            messagebox.showerror("Error","Please select some file !")
        else:
